...
echo -n "Setting password for root..."
passwd
...
useradd -MG wheel,docker -s /usr/bin/zsh -c "$USER_DESCRIPTION" $USER_NAME

# 4. Set User's Password
echo -n "Setting password for user '$USER_NAME'... "
passwd $USER_NAME
...
# 12. Enable SDDM, the Plasma desktop environments display manager.
systemctl enable ssdm

# copy aur.sh to user's home folder
curl -o /home/ariel/finalize.sh -fsSL https://raw.github.com/Latinneo/archer/master/finalize.sh
